...
function getDataTransferImage(event) {
  ...
  try {
    return untrustedHTML ? new DOMParser().parseFromString(untrustedHTML, "text/html").querySelector("img")?.src : dt?.getData("url");
  } catch (e) {
    return;
  }
}
...
function getDataTransferFiles(event) {
  ...
  if (dt) {
    ...
    if ("items" in dt && dt.items.length) {
      return dt.items ? Array.prototype.slice.call(dt.items).filter(dti => dti.kind !== "string").map(dti => dti.getAsFile()).filter(Boolean) : [];
    }
  }
  return [];
}
