...
class MarkdownSerializerState {
  constructor(nodes, marks, options) {
    _defineProperty(this, "inTable", false);
    _defineProperty(this, "inList", false);
    _defineProperty(this, "inTightList", false);
    ...
    const progress = (node, _, index) => {
      ...
      if (node) {
        ...
        if (noEsc && node.isText) {
          const text = this.inTable ? node.text.replace(/\|/gi, "\\$&") : node.text;
          this.text(this.markString(inner, true, parent, index) + text + this.markString(inner, false, parent, index + 1), false);
        } else {
    ...
    node.forEach((row, _, i) => {
      ...
      row.forEach((cell, _, j) => {
        ...
        cell.forEach(cellNode => {
          ...
          if (cellNode.textContent === "" && cellNode.content.size === 0 && cellNode.type.name === "paragraph") {
            this.out += "  ";
          } else {
            this.closed = false;
  ...
  esc() {
    ...
    let startOfLine = arguments.length > 1 ? arguments[1] : undefined;
    str = str.replace(/[`*\\~[\]]/g, "\\$&");
    if (startOfLine) {
