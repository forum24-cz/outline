 ...
 */
const resizeObserverScript = ctx => `<script nonce="${ctx.state.cspNonce}">
 ...
 * origin URL.
 */
const iframeCheckScript = ctx => `<script nonce="${ctx.state.cspNonce}">
 ...
 */
const renderEmbed = async (ctx, next) => {
  const url = (0, _escapeHtml.default)(String(ctx.query.url));
  if (!url) {
  ...
  if (parsed.host === "gitlab.com" && parsed.protocol === "https:" && ctx.path === "/embeds/gitlab") {
    const snippetLink = `${url}.js`;
    const csp = ctx.response.get("Content-Security-Policy");

    // Inject gitlab.com into the script-src and style-src directives
    ctx.set("Content-Security-Policy", csp.replace("script-src", "script-src gitlab.com").replace("style-src", "style-src gitlab.com"));
...
`;
  ...
  if (parsed.host === "gist.github.com" && parsed.protocol === "https:" && ctx.path === "/embeds/github") {
    const id = parsed.pathname.split("/")[2];
    const gistLink = `https://gist.github.com/${id}.js`;
    const csp = ctx.response.get("Content-Security-Policy");

    // Inject GitHub domains into the script-src and style-src directives
    ctx.set("Content-Security-Policy", csp.replace("script-src", "script-src gist.github.com").replace("style-src", "style-src github.githubassets.com"));
    ctx.set("X-Frame-Options", "sameorigin");
    ctx.type = "html";
    ctx.body = `
<html>
...
`;
  ...
  if (parsed.host === "www.dropbox.com" && parsed.protocol === "https:" && ctx.path === "/embeds/dropbox") {
    const dropboxJs = "https://www.dropbox.com/static/api/2/dropins.js";
    const csp = ctx.response.get("Content-Security-Policy");

    // Inject Dropbox domain into the script-src directive
    ctx.set("Content-Security-Policy", csp.replace("script-src", "script-src www.dropbox.com"));
    ctx.set("X-Frame-Options", "sameorigin");
    ctx.type = "html";
    ctx.body = `
<html>
...
id="dropboxjs" data-app-key="${_env.default.DROPBOX_APP_KEY}"></script>
${resizeObserverScript(ctx)}
</body>
`;
    return;
  }
  if (parsed.host.endsWith("pinterest.com") && parsed.protocol === "https:" && ctx.path === "/embeds/pinterest") {
    const pinterestJs = "https://assets.pinterest.com/js/pinit.js";
    const csp = ctx.response.get("Content-Security-Policy");
    const pathParts = parsed.pathname.split("/").filter(Boolean);
    const isProfile = pathParts.length === 1 || pathParts.length === 2 && pathParts[1].startsWith("_");
    const pinType = isProfile ? "embedUser" : "embedBoard";
    ctx.set("Content-Security-Policy", csp.replace("script-src", "script-src assets.pinterest.com widgets.pinterest.com").replace("style-src", "style-src assets.pinterest.com widgets.pinterest.com"));
    ctx.set("X-Frame-Options", "sameorigin");
    ctx.type = "html";
    ctx.body = `
<html>
